...
AgainStart:
    ...
    if (End == 1) // 게임이 끝났을 경우 
    {
        ...
        printf("\n\n");
        printf("                                       Y    /      N                                          \n");
        UserAgain = getch(); // 사용자의 리플레이 여부 입력 받음

        if (UserAgain == 'Y' || UserAgain == 'y') // 만약 플레이 한다면 82번 줄의 AgainStart로 goto
        {
            End = 0;
            Score = 0;
            Sleep(100);
        ...
        else if (UserAgain == 'N' || UserAgain == 'n') // 만약 리플레이 의사 X에는 콘솔창 종료
        {
            exit(1);
        }
    }
